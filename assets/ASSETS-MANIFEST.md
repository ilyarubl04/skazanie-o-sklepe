...
> Все фоны — Unsplash License: бесплатно для коммерции, атрибуция не требуется. `dawn.png` = та же картинка, что `victory.jpg` (код грузит `dawn.png` для финальной сцены). `crypt.png` в коде используется и для сцены босса.
